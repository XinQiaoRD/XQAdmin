...
(function($) {
    $.fn.pekeUpload = function(options) {
        ...
        var defaults = {
            ...
            onFileSuccess: function(file, data, index) {},
            onDelFile: function(file, index){},
            checkWH:""
        };
        ...
        var pekeUpload = {
            ...
            replacehtml: function() {
                ...
                switch (options.dragMode) {
                  ...
                  case false:
                    switch (options.bootstrap) {
                      case true:
                        html = '<a href="javascript:void(0)" class="btn btn-primary btn-upload pkuparea"> <i class="icon-cloud-upload"></i> ' + options.btnText + "</a>";
                        break;
            ...
            addRow: function(file) {
                ...
                switch (options.bootstrap) {
                  case true:
                    var newRow = $('<div class="row pkrw pkRow'+i+'" rel="' + i + '"></div>').appendTo(this.container);
                    if (options.showPreview) {
                        var prev = $('<div class="col-lg-2 col-md-2 col-xs-4"></div>').appendTo(newRow);
                        this.previewFile(prev, file, i);
                    }
                    ...
                    $('<a href="javascript:void(0);" data-file="'+file.name+'" class="btn btn-danger pkdel pkdelIndex'+i+'">'+options.delfiletext+'</a>').appendTo(dismiss);
                    break;
            ...
            previewFile: function(container, file, index) {
                ...
                switch (type) {
                  case "image":
                    ...
                    if(options.checkWH) {
                        var checImage = $('<img src="' + fileUrl + '" />');
                        options.checkWH(checImage, index);
                    }
                    break;
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data == 1 || data.success == 1) {
                            ...
                            options.onFileError(file, err, data.error);
                        }
                    },
